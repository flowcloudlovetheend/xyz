...
复制复杂链表
 */
public class Code3_CopyLink {

    Map<LinkNode, LinkNode> nodeLinkNodeMap = new HashMap<>();
     ...
     */
    public LinkNode repeatLink1(LinkNode head) {
        if (head == null) {
            return null;
        }
        if (!nodeLinkNodeMap.containsKey(head)) {
            LinkNode newNode = new LinkNode(head.num);
            nodeLinkNodeMap.put(head, newNode);
            newNode.next = repeatLink1(head.next);
            newNode.random = repeatLink1(head.random);
        }
        return nodeLinkNodeMap.get(head);
    }
